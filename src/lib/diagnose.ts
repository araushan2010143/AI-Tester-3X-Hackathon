import type { DiagnoseRequest, DiagnosisResult, FailureCluster, FlakyDiagnoseRequest, Framework, LLMProvider, RunHistoryStats } from "./types";
import { FRAMEWORK_LABELS, RUN_PATTERN_LABELS } from "./types";
import type { PrDiff } from "./github";
import { callLLMWithFallback } from "./llm-router";
 ...
 * asked for — the model's job is to explain *why*, grounded in the pattern and
...
export interface DiagnosisWithProvider {
...
export async function diagnoseFailure(
  ...
  prDiff?: PrDiff
): Promise<DiagnosisWithProvider> {
  return callLLMWithFallback(buildDiagnosisPrompt(req, Boolean(screenshotDataUrl), prDiff), screenshotDataUrl);
}

export async function diagnoseCluster(cluster: FailureCluster, framework: Framework): Promise<DiagnosisWithProvider> {
  return callLLMWithFallback(buildBulkDiagnosisPrompt(cluster, framework));
}

export async function diagnoseFlakiness(stats: RunHistoryStats, req: FlakyDiagnoseRequest): Promise<DiagnosisWithProvider> {
  return callLLMWithFallback(buildFlakyDiagnosisPrompt(stats, req));
}

export { GeminiNotConfiguredError, GeminiOverloadedError } from "./gemini-provider";
export { AllProvidersUnavailableError } from "./llm-router";
